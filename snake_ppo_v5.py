"""
snake_ppo_v5.py  -  PPO + Torus CNN + Fine Curriculum
=====================================================================
Changes from v4
  - TorusConv2d: wraps grid edges before each conv so the network sees
    edge cells as genuinely adjacent - fixes wrap-collision deaths
  - Updated curriculum array (12-stage, 2-row increments, tuned shaping)

Architecture
  Input  : [B, 4, 40, 40]  float32
  Trunk  : TorusConv(4->32) -> TorusConv(32->64) -> TorusConv(64->64)
...
EVAL_INTERVAL   = 5
SAVE_INTERVAL   = 100
CHECKPOINT_PATH = "snake_ppo_v5_ckpt.pt"
...
# ===============================================================================
#  2.  CURRICULUM MANAGER  (identical to v4)
# ===============================================================================
...
class TorusConv2d(nn.Module):
    """
    Conv2d with torus (circular) padding.
    Wraps left↔right and top↔bottom so edge cells are treated as neighbours.
    Fixes the wrap-collision problem where the CNN sees opposite edges
    as spatially distant when they are actually adjacent.
    """
    ...
    def forward(self, x):
        p = self.pad
        # Wrap horizontally then vertically
...
class ActorCriticCNN(nn.Module):
    """
    Torus-aware CNN trunk + actor/critic heads.
    TorusConv2d wraps grid edges before each conv so opposite edges
    are treated as spatially adjacent - fixes wrap-collision deaths.
    """
...
# ===============================================================================
#  4.  ROLLOUT BUFFER  (identical to v4)
# ===============================================================================
...
class PPOTrainer:
    ...
    def _on_graduate(self):
        ...
        ckpt = f"snake_ppo_v5_ckpt_stage{prev}.pt"
        self.save(ckpt)
        print(f"\n{'='*60}")
...
def draw_panel(screen, trainer: PPOTrainer, metrics: dict, snake: LiveSnake):
    ...
    y = 16
    screen.blit(f_title.render("SNAKE  PPO  v5", True, ACCENT), (px+12, y))
    y += 24
...
def main():
    ...
    pygame.display.set_caption("Snake - PPO v5 (Torus CNN)")
    ...
    print(f"\nSnake PPO v5  |  envs={NUM_ENVS}  T={ROLLOUT_STEPS}  "
          f"batch={NUM_ENVS*ROLLOUT_STEPS:,}  device={device}")
    print(f"Curriculum : {stages_str}")
    print(f"Model      : TorusCNN -> Linear(512) -> Actor/Critic")
