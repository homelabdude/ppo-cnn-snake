"""
snake_ppo_v2.py  -  PPO + CNN + Curriculum Learning
=====================================================================
Changes from v1
  - CurriculumManager: performance-based grid graduation
      10×10 -> 20×20  when avg_score > 5.0 over last 100 episodes
      20×20 -> 40×40  when avg_score > 8.0 over last 100 episodes
  - SnakeEnv now takes grid_size as a parameter
  - Observations always padded to 40×40 - model architecture never changes
  - Checkpoint saved at each graduation so you can always roll back
  - Torus-correct Manhattan distance shaping (0.03 weight)
  - ENTROPY_COEF 0.05, LR 3e-5, CLIP_EPS 0.1 (stabilised from v1)
  - HUD shows current curriculum stage

Everything else identical to v1 - same model, same PPO, same rollout buffer.
To revert to a previous stage load snake_ppo_ckpt_stage0.pt / stage1.pt etc.

Architecture (unchanged from v1)
  Input  : [B, 4, 40, 40]  float32  (padded if grid < 40)
  Trunk  : Conv2d 4->32->64->64  k=3 pad=1
           Flatten -> Linear(64·40·40, 512) -> ReLU
  Heads  : actor Linear(512,3)   critic Linear(512,1)

Reward   : +1.0 eat   −1.0 die   ±0.03 distance shaping (torus)
"""
...
# -- Display constants (fixed - always render at 40×40 scale) -----------------
WIDTH      = 600
HEIGHT     = 600
GRID_SIZE  = 15          # pixel size of one cell at full 40×40 grid
...
GAMMA           = 0.99
GAE_LAMBDA      = 0.95
CLIP_EPS        = 0.1          # tightened from v1
VALUE_COEF      = 0.5
ENTROPY_COEF    = 0.05         # higher than v1 - keep exploring longer
LR              = 3e-5         # lower than v1 - more stable updates
MAX_GRAD_NORM   = 0.5
PPO_EPOCHS      = 2
MINIBATCH_SIZE  = 512
...
EVAL_INTERVAL   = 5
SAVE_INTERVAL   = 100
CHECKPOINT_PATH = "snake_ppo_v2_ckpt.pt"
...
# -- Curriculum ----------------------------------------------------------------
CURRICULUM_STAGES = [
    {"grid": 10, "graduate_score": 5.0,  "label": "Stage 0 - 10×10"},
    {"grid": 20, "graduate_score": 8.0,  "label": "Stage 1 - 20×20"},
    {"grid": 40, "graduate_score": None, "label": "Stage 2 - 40×40"},
]
GRAD_WINDOW = 100    # episodes to average over for graduation check
...
def torus_dist(ax, ay, bx, by, cells):
    """Manhattan distance on a wrapping torus of size `cells`."""
    dx = abs(ax - bx)
    dy = abs(ay - by)
...
class SnakeEnv:
    """
    Single Snake env parameterised by grid_size (10, 20, or 40).
    Observation is always padded to [4, FULL_CELLS, FULL_CELLS] so the
    model never needs to change regardless of curriculum stage.
    """
    ...
    def step(self, action: int):
        ...
        # Distance shaping - torus-correct
        old_d  = torus_dist(hx, hy, self.fruit[0], self.fruit[1], g)
        new_d  = torus_dist(new_head[0], new_head[1], self.fruit[0], self.fruit[1], g)
        reward = 0.03 * (old_d - new_d)
    ...
    def _obs(self) -> np.ndarray:
        """
        Build obs on the active grid, then pad to [4, FULL_CELLS, FULL_CELLS].
        Padding puts the active grid in the top-left corner - consistent
        across all stages so the model can always find the snake at the
        same spatial address relative to the origin.
        """
        g   = self.grid_size
        ...
        # Direction channel - only fill active grid region
...
class VecSnakeEnv:
    ...
    def set_grid_size(self, grid_size: int):
        """Called by CurriculumManager on graduation - rebuilds all envs."""
        self.grid_size = grid_size
...
class CurriculumManager:
    """
    Watches rolling avg score and triggers grid graduation.
    Completely decoupled from the model - only talks to VecSnakeEnv.
    """

    def __init__(self):
        self.stage        = 0
        self.score_window = deque(maxlen=GRAD_WINDOW)
        self.graduated    = False   # True once we hit 40×40
    ...
    def record_episode(self, score: int) -> bool:
        ...
        if (threshold is not None
                ...
                and (sum(self.score_window) / GRAD_WINDOW) >= threshold):
            self.stage += 1
...
# ===============================================================================
#  3.  MODEL  (identical to v1 - architecture never changes)
# ===============================================================================
...
# ===============================================================================
#  4.  ROLLOUT BUFFER  (identical to v1)
# ===============================================================================
...
class PPOTrainer:
    def __init__(self, device: torch.device):
        ...
        # Per-stage score history for sparkline
        ...
        for _ in range(ROLLOUT_STEPS):
            ...
            for i, done in enumerate(dones):
                if done:
                    ...
                    # Feed score to curriculum
                    just_graduated = self.curriculum.record_episode(ep_score)
                    if just_graduated and not graduated:
                        graduated = True
    ...
    def _on_graduate(self):
        ...
        label = self.curriculum.label
        ckpt  = f"snake_ppo_v2_ckpt_stage{stage - 1}.pt"
        self.save(ckpt)
        print(f"\n{'='*60}")
        print(f"  GRADUATION -> {label}")
        print(f"  Rollback checkpoint saved: {ckpt}")
        print(f"{'='*60}\n")
        # Rebuild envs at new grid size - model weights carry over unchanged
...
class LiveSnake:
    ...
    def render(self, surface, cell_px: int):
        """cell_px = pixel size of one cell for the current grid."""
        env = self.env
        g   = env.grid_size

        # Draw active grid boundary
        pygame.draw.rect(surface, (50, 50, 70),
                         (0, 0, g * cell_px, g * cell_px), 1)
        ...
        for idx, (bx, by) in enumerate(env.body):
            ...
            if idx == 0:
                ...
                pygame.draw.circle(surface, col, (px, py), cell_px // 2)
...
def draw_panel(screen, trainer: PPOTrainer, metrics: dict, snake: LiveSnake):
    ...
    y = 16
    screen.blit(f_title.render("SNAKE  PPO  v2", True, ACCENT), (px + 12, y))
    y += 24
...
def run_eval_episode(trainer: PPOTrainer, screen, surface, clock,
    ...
    cell_px = WIDTH // g   # bigger cells on small grids - snake is easy to see
    ...
    while not snake.done:
        ...
        surface.fill(GREY)
        # Grid dots - sized to current grid
...
def main():
    ...
    pygame.display.set_caption("Snake - PPO CNN v2 (Curriculum)")
    ...
    print(f"\nSnake PPO v2  |  envs={NUM_ENVS}  T={ROLLOUT_STEPS}  "
          f"batch={NUM_ENVS*ROLLOUT_STEPS:,}  device={device}")
    print(f"Curriculum: 10×10 -> 20×20 -> 40×40  (performance-based)")
    ...
    while trainer.total_steps_done < TOTAL_STEPS:
        ...
        stage  = trainer.curriculum.label
