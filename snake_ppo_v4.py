"""
snake_ppo_v4.py  -  PPO + CNN + Fine Curriculum + Soft Shaping
=====================================================================
Changes from v3
  - 10-stage curriculum with 2-row grid increments - max ~1.6x area per jump
      8×8 -> 10×10 -> 12×12 -> 15×15 -> 18×18 -> 20×20 -> 22x22
           -> 25×25 -> 28x28 -> 30×30 -> 35×35 -> 40×40
  - Per-stage distance shaping weight - tiny, tapers to 0 at final stage
      8,10        -> 0.000  (dense enough, pure sparse)
      12,15       -> 0.005  (first shaping, soft landing after density drop)
      18,20,22,25 -> 0.004
      28,32       -> 0.003
      36          -> 0.002
      40          -> 0.000  (pure sparse at final stage)
  - Revisit penalty carried throughout all stages (unchanged from v3)
  - Same model architecture, same PPO hyperparams

Reward per step:
  +1.0                           eat fruit
  -1.0                           die
  shaping_weight * (old_d-new_d) torus distance delta (0 at stages 0,1,9)
  -revisit_penalty               if cell was visited before (scales w/ length)

Architecture (identical throughout all versions)
  Input  : [B, 4, 40, 40]  float32  (active grid top-left padded)
...
EVAL_INTERVAL   = 5
SAVE_INTERVAL   = 100
CHECKPOINT_PATH = "snake_ppo_v4_ckpt.pt"
...
# -- Curriculum ----------------------------------------------------------------
# shaping_weight: torus distance delta coefficient for this stage
# 0.0 = pure sparse, >0 = soft landing pad after grid jump
CURRICULUM_STAGES = [
...
class SnakeEnv:
    """
    Single Snake env parameterised by grid_size and shaping_weight.
    Torus wrapping throughout.
    ...
    def step(self, action: int):
        ...
        # Soft distance shaping (0.0 at pure-sparse stages)
        ...
        if new_head == self.fruit:
            ...
            self.fruit         = self._new_fruit()
            reward             = 1.0   # eat overrides everything
        else:
            if len(self.body) > self.length:
                self.body.pop()
...
class VecSnakeEnv:
    ...
    def set_stage(self, grid_size: int, shaping_weight: float):
        """Called at graduation - rebuilds all envs with new grid and shaping."""
        self.grid_size      = grid_size
        self.shaping_weight = shaping_weight
...
class CurriculumManager:
    ...
    @property
    def grid_size(self):
        return self.current_stage["grid"]

    @property
    def shaping_weight(self):
        return self.current_stage["shaping"]
...
# ===============================================================================
#  3.  MODEL  (identical throughout all versions)
# ===============================================================================
...
class PPOTrainer:
    ...
    def _on_graduate(self):
        ...
        ckpt  = f"snake_ppo_v4_ckpt_stage{prev}.pt"
        self.save(ckpt)
        print(f"\n{'='*60}")
...
def draw_panel(screen, trainer: PPOTrainer, metrics: dict, snake: LiveSnake):
    ...
    y = 16
    screen.blit(f_title.render("SNAKE  PPO  v4", True, ACCENT), (px+12, y))
    y += 24
...
def main():
    ...
    pygame.display.set_caption("Snake - PPO CNN v4 (Fine Curriculum)")
    ...
    print(f"\nSnake PPO v4  |  envs={NUM_ENVS}  T={ROLLOUT_STEPS}  "
          f"batch={NUM_ENVS*ROLLOUT_STEPS:,}  device={device}")
    print(f"Curriculum : {stages_str}")
    print(f"Shaping    : per-stage soft landing (0.005 -> 0.0 at final stage)")
    print(f"Revisit    : 0.002 × visits^1.2 × max(1, length/5)")
    print("-" * 60)
