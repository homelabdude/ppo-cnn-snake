"""
snake_ppo_v6.py  —  PPO + Torus CNN + 4-Frame Stack + Fine Curriculum
=====================================================================
Changes from v5
  - Frame stacking: last 4 observations stacked -> [16, 40, 40] input
    Gives the CNN implicit temporal memory:
      frames 0-3: head positions over last 4 steps  -> movement direction
      frames 4-7: body positions over last 4 steps  -> body trajectory
      frames 8-11: fruit positions                  -> fruit stability check
      frames 12-15: direction channels              -> heading history
  - Model input changes: TorusConv2d(4->32) -> TorusConv2d(16->32)
  - SnakeEnv maintains a deque of last N_FRAMES single observations
  - Everything else identical to v5

Architecture
  Input  : [B, 16, 40, 40]  float32  (4 frames × 4 channels)
  Trunk  : TorusConv(16->32) -> TorusConv(32->64) -> TorusConv(64->64)
           Flatten -> Linear(64·40·40, 512) -> ReLU
  Heads  : actor Linear(512, 3)   critic Linear(512, 1)

Reward   : +1.0 eat   −1.0 die   shaping·Δdist   −revisit_penalty
"""
...
N_CHANNELS = 4    # channels per frame
OBS_CHANNELS = N_FRAMES * N_CHANNELS   # 16 total input channels
...
EVAL_INTERVAL   = 5
SAVE_INTERVAL   = 100
CHECKPOINT_PATH = "snake_ppo_v6_ckpt.pt"

# -- Anti-looping --------------------------------------------------------------
# Scales with grid size — larger grids need more steps to find fruit
# 8×8->28×28: floor of 200, 32×32: 256, 36×36: 324, 40×40: 400
# Handled dynamically in SnakeEnv.reset()
...
# ===============================================================================
#  1.  ENVIRONMENT  — adds frame stacking
# ===============================================================================

class SnakeEnv:
    """
    Snake env with N_FRAMES frame stacking.
    Returns [N_FRAMES*4, FULL_CELLS, FULL_CELLS] stacked observation.
    Most recent frame is channels 0-3, oldest is channels 12-15.
    """
    ...
    def _single_obs(self) -> np.ndarray:
        """Single frame [4, FULL_CELLS, FULL_CELLS]."""
        g   = self.grid_size
    ...
    def _stacked_obs(self) -> np.ndarray:
        """Stack frame buffer -> [16, FULL_CELLS, FULL_CELLS]."""
    ...
    def reset(self):
        ...
        # Dynamic step limit — scales with grid size
        self.max_steps_no_eat = max(200, g * g // 4)
        # Frame buffer — initialise all frames with same first obs
        first = self._single_obs()
        self.frame_buffer = deque([first.copy() for _ in range(N_FRAMES)],
                                  maxlen=N_FRAMES)
    ...
    def step(self, action: int):
        ...
        if new_head in body_set:
            self.done = True
            # Push final frame before returning
            self.frame_buffer.append(self._single_obs())
            return self._stacked_obs(), -1.0, True, {"score": self.score}
        ...
        self.frame_buffer.append(self._single_obs())
        return self._stacked_obs(), reward, False, {"score": self.score}
...
# ===============================================================================
#  2.  CURRICULUM MANAGER  (identical to v5)
# ===============================================================================
...
# ===============================================================================
#  3.  MODEL  — Torus CNN with 16-channel input (4-frame stack)
# ===============================================================================

class TorusConv2d(nn.Module):
    """Conv2d with torus (circular) padding — identical to v5."""
...
class ActorCriticCNN(nn.Module):
    """
    Torus CNN with 16-channel input (4 stacked frames × 4 channels).
    The CNN sees the last 4 game states simultaneously — enabling it to
    infer movement direction, detect loops, and reason about body trajectory
    without any recurrence.
    """

    def __init__(self):
        super().__init__()
        self.conv1  = TorusConv2d(OBS_CHANNELS, 32)   # 16 -> 32
        self.conv2  = TorusConv2d(32, 64)
        self.conv3  = TorusConv2d(64, 64)
        self.relu   = nn.ReLU()
        cnn_out     = 64 * FULL_CELLS * FULL_CELLS
        self.linear = nn.Linear(cnn_out, 512)
...
# ===============================================================================
#  4.  ROLLOUT BUFFER  (identical to v5, obs shape updated automatically)
# ===============================================================================
...
class PPOTrainer:
    def __init__(self, device: torch.device):
        ...
        self.buffer    = RolloutBuffer(ROLLOUT_STEPS, NUM_ENVS,
                                       (OBS_CHANNELS, FULL_CELLS, FULL_CELLS))
    ...
    def _on_graduate(self):
        ...
        ckpt = f"snake_ppo_v6_ckpt_stage{prev}.pt"
        self.save(ckpt)
        print(f"\n{'='*60}")
    ...
    def load(self, path: str = CHECKPOINT_PATH):
        ...
        print(f"  [ckpt ← {path}  steps={self.total_steps_done:,}  "
              ...
              f"shaping={self.curriculum.shaping_weight}]")
...
def draw_panel(screen, trainer: PPOTrainer, metrics: dict, snake: LiveSnake):
    ...
    y = 16
    screen.blit(f_title.render("SNAKE  PPO  v6", True, ACCENT), (px+12, y))
    y += 24
...
def main():
    ...
    pygame.display.set_caption("Snake — PPO v6 (Torus CNN + Frame Stack)")
    ...
    print(f"\nSnake PPO v6  |  envs={NUM_ENVS}  T={ROLLOUT_STEPS}  "
          f"batch={NUM_ENVS*ROLLOUT_STEPS:,}  device={device}")
    print(f"Curriculum : {stages_str}")
    print(f"Model      : TorusCNN(16ch) -> Linear(512) -> Actor/Critic")
    print(f"Input      : {N_FRAMES} frames × {N_CHANNELS} channels = {OBS_CHANNELS} channels")
    print(f"Reward     : sparse +1/-1  +  shaping  +  revisit penalty")
    print("-" * 60)
